...
def _xrf_icp_table(options: List[str]) -> str:
    ...
    if "ICP-OES" in icp_columns or "ICP-MS" in icp_columns:
        ...
        rows = [
            ["آماده‌سازی نمونه", "کم؛ معمولاً بدون هضم", "نیازمند آماده‌سازی یا هضم" if "ICP-OES" in icp_columns else None, "نیازمند آماده‌سازی بسیار کنترل‌شده" if "ICP-MS" in icp_columns else None],
            ["حد تشخیص نسبی", "مناسب‌تر برای غلظت‌های بالاتر", "پایین‌تر از XRF" if "ICP-OES" in icp_columns else None, "بسیار پایین‌تر؛ مناسب trace/ultra-trace" if "ICP-MS" in icp_columns else None],
            ["دقت و صحت", "وابسته به ماتریس و کالیبراسیون", "بالا برای کار چندعنصری" if "ICP-OES" in icp_columns else None, "بسیار بالا در غلظت‌های پایین" if "ICP-MS" in icp_columns else None],
            ["سرعت و سادگی", "سریع و ساده", "زمان‌برتر از XRF" if "ICP-OES" in icp_columns else None, "زمان‌برتر و حساس‌تر به آلودگی" if "ICP-MS" in icp_columns else None],
            ["محدودیت اصلی", "اثر ماتریس و یکنواختی نمونه", "هضم ناقص و تداخل طیفی" if "ICP-OES" in icp_columns else None, "آلودگی، تداخل جرمی و هزینه بالاتر" if "ICP-MS" in icp_columns else None],
            ["بهترین کاربرد", "غربالگری سریع", "گزارش دقیق چندعنصری" if "ICP-OES" in icp_columns else None, "فلزات در غلظت بسیار پایین" if "ICP-MS" in icp_columns else None],
        ]
    ...
    return _build_markdown_table(
        ["معیار", "XRF", "ICP"],
        [
            ["آماده‌سازی نمونه", "کم؛ معمولاً بدون هضم", "نیازمند آماده‌سازی یا هضم اسیدی"],
            ["حد تشخیص نسبی", "مناسب‌تر برای غلظت‌های بالاتر", "پایین‌تر؛ مناسب‌تر برای trace metals"],
            ["دقت و صحت", "وابسته به ماتریس و کالیبراسیون", "معمولاً دقیق‌تر برای گزارش رسمی"],
            ["سرعت و سادگی", "سریع‌تر و ساده‌تر", "زمان‌برتر و وابسته به آماده‌سازی"],
            ["محدودیت اصلی", "اثر ماتریس، ضخامت و یکنواختی نمونه", "هضم ناقص، آلودگی و تداخل طیفی"],
            ["بهترین کاربرد", "غربالگری سریع و کنترل روتین", "اندازه‌گیری دقیق چندعنصری"],
        ],
    )
...
def _gc_hplc_table() -> str:
    return _build_markdown_table(
        ...
        [
            ...
            ["آماده‌سازی نمونه", "وابسته به فراریت و تمیزی نمونه", "وابسته به حلالیت و فیلتراسیون"],
            ["محدودیت اصلی", "نامناسب برای ترکیبات غیر فرار", "مصرف حلال و حساسیت به ماتریس"],
            ["بهترین کاربرد", "گازها، حلال‌ها، VOC و BTEX", "مواد سنگین‌تر، افزودنی‌ها و ترکیبات قطبی"],
        ],
    )
...
def _icp_aas_table() -> str:
    return _build_markdown_table(
        ...
        [
            ...
            ["حد تشخیص نسبی", "خوب برای بسیاری از فلزات", "معمولاً پایین‌تر؛ به‌ویژه در ICP-MS"],
            ["سرعت", "کندتر برای چند عنصر", "سریع‌تر برای چند عنصر"],
            ["هزینه", "کمتر", "بالاتر"],
            ["بهترین کاربرد", "تعداد کم عنصر و بودجه محدود", "پروفایل کامل فلزات و گزارش دقیق"],
        ],
    )
...
def ensure_comparison_table(message: str, answer: str) -> str:
    ...
    for pattern in marker_patterns:
        ...
        if match:
            return answer[: match.start()].rstrip() + section + "\n" + answer[match.start():].lstrip()
