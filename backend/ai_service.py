...
_WEB_SEARCH_DIRECTIVE = (
    ...
    "جایگزین‌شده)، مشخصات کلیدی، و مدل‌های جایگزین.\n"
    "- قیمت نده. اگر کاربر قیمت خواست، بگو برای قیمت و پیش‌فاکتور با کارشناسانِ "
    "آرتین‌آزما تماس بگیرد و قیمتِ عددی از وب نقل نکن.\n"
    ...
    "- «منابع داخلی» (بانک دانش آرتین‌آزما، بخشِ context) معتبرند و اولویت دارند — "
    "به‌ویژه برای استانداردهای ملیِ ایران (INSO/ISIRI) و جدول‌های معادلِ استاندارد که "
    "در وب سخت پیدا می‌شوند. اگر شماره‌ی INSO یا معادلِ استانداردی در منابع داخلی آمده، "
    "همان را مستقیم استفاده کن و برای «تأییدِ وب» ردش نکن؛ وب فقط برای تکمیل است، نه رد "
    "کردنِ منبعِ داخلی. «یافت نشد» را فقط وقتی بگو که نه در منابع داخلی و نه در وب چیزی نبود.\n"
    "- برای ASTM و ISO همیشه «نسخهٔ جاری» را بده و وضعیتِ استاندارد را از سایت رسمی "
    "(store.astm.org / iso.org) چک کن: اگر منسوخ (Withdrawn) یا جایگزین‌شده (Superseded) "
    "است، صریحاً همان را بگو و نسخهٔ جاری/جایگزینِ رسمی را معرفی کن. اگر بانک دانش نسخهٔ "
    "قدیمی دارد، کد را بگیر ولی سال/نسخهٔ روز و فعال‌بودن را از وب تأیید کن — استانداردِ "
    "قدیمی یا منسوخ را به‌عنوانِ «انتخابِ اول» جا نزن.\n"
...
- ادعای منفی هم نساز: نگو یک مدل مشخص فلان قابلیت را «ندارد» مگر مطمئن باشی؛ نبودِ یک قابلیت را فقط وقتی بگو که از منبع یا داده تأیید شده باشد. اگر مطمئن نیستی، تفاوت را نسبی و محتاطانه بیان کن (مثلاً «سریع‌تر/گسترده‌تر» به‌جای «دارد/ندارد»).
- شماره‌ی قطعه، کد لوازم جانبی یا مدل دقیق (مثل کد یک شیلد، سل یا ماژول) را از خودت نساز؛ اگر کد دقیق را از منبع نداری، فقط نوع/کاربرد قطعه را بگو و بخواه از دیتاشیت یا سازنده تأیید شود.
