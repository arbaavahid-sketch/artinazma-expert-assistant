...
def get_specialized_protocol(query_type: str) -> str:
    protocols = {
        "comparison": """
پروتکل مقایسه فنی:
- فقط مزایا و معایب عمومی نگو؛ پاسخ باید تصمیم‌ساز باشد.
- ابتدا تفاوت بنیادی روش‌ها، دستگاه‌ها یا مواد را توضیح بده.
- سپس از نظر اصل عملکرد، آماده‌سازی نمونه، ماتریس، حساسیت، دقت، تداخل‌ها، QC، سرعت، هزینه، اپراتوری، استانداردها و محدودیت‌ها مقایسه کن.
- اگر اصطلاحی چند زیرشاخه دارد، آن را تفکیک کن؛ مثلاً ICP را به ICP-OES و ICP-MS تفکیک کن.
- برای نمونه‌های نفتی، اثر ماتریس، ویسکوزیته، همگن‌سازی، رسوب، آسفالتن، آب، نمک، رقیق‌سازی، هضم، استانداردهای organometallic و استانداردهای مشابه ماتریس را لحاظ کن.
- در پایان سناریوی انتخاب بده: چه زمانی گزینه اول بهتر است و چه زمانی گزینه دوم.
- اگر اطلاعات کاربر ناقص است، اطلاعات لازم برای تصمیم قطعی را مشخص کن.
""",
        "sales": """
پروتکل فروش فنی:
...
def clean_ai_answer(text: str) -> str:
    ...
    # حذف جمله‌های ناقص درباره لینک وقتی لینک وجود ندارد
    has_link = bool(re.search(r"https?://\S+", cleaned)) or bool(
        re.search(r"\[[^\]]+\]\(https?://[^)]+\)", cleaned)
    )
    ...
    if not has_link:
        weak_link_sentences = [
            ...
            r"\n?\s*برای دریافت اطلاعات بیشتر.*?با شرکت تماس بگیرید\.?\s*$",
        ]

        for pattern in weak_link_sentences:
...
دستور:
- پاسخ را متوقف نکن.
- بر اساس دانش فنی عمومی، پاسخ کاربردی، حرفه‌ای و قابل تصمیم‌گیری بده.
- برای قطعیت، اطلاعات لازم مثل دیتاشیت، کاتالوگ، برند، مدل کامل، نوع نمونه، محدوده اندازه‌گیری، حد تشخیص، استاندارد موردنیاز یا شرایط عملیاتی را مشخص کن.
"""
