...
def get_intent_instruction(intent: str) -> str:
    instructions = {
        ...
        فقط وقتی منبع یا کانتکست معتبر تماس همراه درخواست وجود دارد، ایمیل یا شماره دقیق را ذکر کن؛ در غیر این صورت کاربر را به فرم درخواست یا اطلاعات تماس رسمی سایت هدایت کن.
        """,
        "method_comparison": """
        ...
        جدول مقایسه سریع الزامی است.
        جدول باید Markdown واقعی باشد و دقیقاً با این ساختار ساخته شود:
        ...
        قواعد جدول:
        - نام ستون‌های گزینه اول و گزینه دوم را با نام واقعی روش‌ها جایگزین کن؛ مثلاً XRF و ICP.
        - اگر بیش از دو گزینه مقایسه می‌شود، ستون سوم اضافه کن؛ مثلاً ICP-OES و ICP-MS.
        - متن هر سلول کوتاه باشد؛ حداکثر یک عبارت یا یک جمله کوتاه.
        - عدد، LOD، LOQ یا استاندارد خاص نساز مگر داده قطعی در context وجود داشته باشد.
        - بعد از جدول، یک جمع‌بندی انتخابی کوتاه بده.
        ...
        - پاسخ را تصمیم‌ساز بنویس، نه مقاله‌ای.
