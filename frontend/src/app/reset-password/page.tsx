...
function ResetPasswordInner() {
  ...
  async function handleReset() {
    setMessage("");
    if (!newPassword.trim()) { setMessage("رمز عبور جدید را وارد کنید."); return; }
    if (newPassword.length < 6) { setMessage("رمز عبور باید حداقل ۶ کاراکتر باشد."); return; }
    if (newPassword !== confirmPassword) { setMessage("رمز عبور و تکرار آن یکسان نیستند."); return; }
    ...
    return (
      <section className="min-h-full bg-[#f7f7f8] px-6 py-8">
        <div className="mx-auto flex min-h-[calc(100vh-64px)] max-w-lg items-center justify-center">
          <div className="text-slate-500">در حال بررسی لینک...</div>
        </div>
      </section>
    );
  }
      ...
      <section className="min-h-full bg-[#f7f7f8] px-6 py-8">
        <div className="mx-auto flex min-h-[calc(100vh-64px)] max-w-lg items-center justify-center">
          <div className="ui-card w-full rounded-[32px] p-8 text-center shadow-sm">
            ...
            <h2 className="text-xl font-black text-slate-900">لینک نامعتبر</h2>
            <p className="mt-3 leading-8 text-slate-600">این لینک بازیابی منقضی شده یا قبلاً استفاده شده است.</p>
    ...
    <section className="min-h-full bg-[#f7f7f8] px-6 py-8">
      <div className="mx-auto flex min-h-[calc(100vh-64px)] max-w-lg items-center justify-center">
        <div className="ui-card w-full rounded-[32px] p-8 shadow-sm lg:p-10">
          ...
          <h1 className="text-2xl font-black leading-[1.5] text-slate-900">رمز عبور جدید</h1>

          {!success ? (
            <>
              <div className="mt-6 space-y-4">
                <div>
                  <label className="mb-2 block text-sm font-bold text-slate-700">رمز عبور جدید</label>
                ...
                <div>
                  <label className="mb-2 block text-sm font-bold text-slate-700">تکرار رمز عبور</label>
                  <div className="relative">
                    <LockKeyhole size={18} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400" />
                    <input
                      type="password"
